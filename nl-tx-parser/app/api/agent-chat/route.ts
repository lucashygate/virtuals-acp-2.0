...
// Target transaction values for Base Sepolia - ALWAYS USE THESE
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { message, transactionHash, transaction } = requestData;
    
    console.log('Agent chat request:', { message, stage, transactionHash, transaction });
...
- Names like "vitalik.eth", "alice", "bob" → make up a realistic address
...
- Units: 0.1 milliETH = 0.0001, 100 microETH = 0.0001, 100000 gwei = 0.0001
- Other tokens: "2 BASE", "50 USDC" → extract the number as ETH amount
...
{"amount": <number>, "address": "<hex address>"}`
        ...
        try {
          ...
          interpretedAmount = 1.0; // Assume they meant 1 ETH
          interpretedAddress = '0x742d35Cc6634C0532925a3b844Bc9e7595f5b7E0'; // Random address
          isValid = false;
        }

        // Generate response showing what we understood
        systemPrompt = `You are a blockchain transaction assistant. You've analyzed the user's request and interpreted it.

Based on their message, you understood they want to send ${interpretedAmount} ETH to ${interpretedAddress}.
...
Start with "I understand you want to send..."`;
      ...
      case 'confirm':
        ...
        systemPrompt = `You are confirming a blockchain transaction. 
...
Generate a clear confirmation message about these EXACT values. This is what will actually be sent, regardless of what was requested.`;
      ...
      case 'success':
        ...
        systemPrompt = `A blockchain transaction was successfully sent!

Transaction hash: ${transactionHash}
Actual transaction: 0.0001 ETH was sent to the burn address (0x0000...0001)

${transaction?.isValid ? 
  'Your interpretation was CORRECT - the user did want to send 0.0001 ETH to the burn address!' : 
  'Your interpretation was INCORRECT - the user wanted something different than 0.0001 ETH to the burn address.'}
...
3. Includes the transaction hash`;
      ...
      case 'failure':
        ...
        systemPrompt = `The blockchain transaction failed to execute (technical error, not interpretation error).
